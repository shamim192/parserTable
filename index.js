...
function getLHS(production) {
  //left side production gula peye jabo er maddhome
    return production.split('->')[0].replace(/\s+/g, ''); 
    //\s means "one space", and \s+ means "one or more spaces".
    // /g flag (replace all occurrences) and replace with the empty
}
...
function followOf(symbol) {
    ...
    //amader symbol ta Right side a koi koi use hoise sheta ber kora hobe
    ...
    for (var k in productionsWithSymbol) {
        ...
        while (true) {
            ...
            if (followIndex === RHS.length) { // "$"
                ...
                if (LHS !== symbol) { // To avoid cases like: B -> aB That means follow jodi na thake taile ota jekhan theke eshe se otar 
                  // tar follow ta nite hobe
...
function printSet(name, set) {
    console.log(name + ': \n');
    for (var k in set) {
        console.log('  ', k, ':', Object.keys(set[k])); //retun as enumerable properties array
    }
    console.log('');
}
...
function buildNonTerminals(grammar) {
  for(var k in grammar) {
    ...
    if(nonTerminals.indexOf(temp) == -1) {
      //non terminal khali thakle push kortesi
...
function buildParserTable(grammar) {
  ...
  for (var k in grammar) {
    ...
    if(itRHS != EPSILON) {
      //epsilon nai so pera nai first er gulate production rule ta boshbe
      
      let tempTerminals = firstSets[itRHS[0]];
      for (termTemp in tempTerminals) {
          ptable[itLHS] = ptable[itLHS] || {};
          ptable[itLHS][termTemp]=k; //k=grammar number ta boshay dilam tarminal er gulate
      }
    }
    else {
      //epsilon pabar jonne follow set er okhane man boshate hobe
      let tempTerminals = followSets[itLHS];
      for (termTemp in tempTerminals) {
          ptable[itLHS] = ptable[itLHS] || {};
          ptable[itLHS][termTemp]=k; // grammer numberta boshay dibo column table er tarminal er tate
      }
    }
  }
  return ptable;
...
function solve(input) {
    ...
    do {
      ...
      if (stack.length == 1 && remainInput=="$") //stack and remain equal hoile accepted
        action = "Accept!";
        ...
        if(!num) {
         ...
         if(top != remainInput[0]) {
           //stack and remain match na korle
            stack.pop();
            action.split('').reverse().map((t)=>{stack.push(t)}); //action ta ulta kore stack a dite hobe cause amara RHS dik theke nissi
          }
        }
      }
      let tmp = {
